...
// 测试预解析后的性能
func BenchmarkValidateEnumParsed(b *testing.B) {
	// 预解析规则（模拟只执行一次的解析）
	simpleEnum := &parser.ParsedEnumValue{
		ExactValues: map[string]struct{}{"5": {}},
		RawRule:     "5",
	}
	rangeEnum := &parser.ParsedEnumValue{
		Ranges:  []parser.EnumRange{{Min: 1, Max: 41}},
		RawRule: "1-41",
	}
	mixedEnum := &parser.ParsedEnumValue{
		ExactValues: map[string]struct{}{"9999": {}},
		Ranges:      []parser.EnumRange{{Min: 1, Max: 41}},
		RawRule:     "1-41,9999",
	}

	// 测试简单值 - 预解析
	b.Run("简单值_预解析", func(b *testing.B) {
		rv := &RuleValidator{FieldValue: "5", ParsedEnum: simpleEnum}
		for i := 0; i < b.N; i++ {
			rv.validateEnumParsed(simpleEnum)
		}
	})

	// 测试范围 - 预解析
	b.Run("范围_预解析", func(b *testing.B) {
		rv := &RuleValidator{FieldValue: "5", ParsedEnum: rangeEnum}
		for i := 0; i < b.N; i++ {
			rv.validateEnumParsed(rangeEnum)
		}
	})

	// 测试混合格式 - 预解析
	b.Run("混合格式_预解析", func(b *testing.B) {
		rv := &RuleValidator{FieldValue: "5", ParsedEnum: mixedEnum}
		for i := 0; i < b.N; i++ {
			rv.validateEnumParsed(mixedEnum)
		}
	})

	// 测试不匹配 - 预解析
	b.Run("不匹配_预解析", func(b *testing.B) {
		rv := &RuleValidator{FieldValue: "100", ParsedEnum: mixedEnum}
		for i := 0; i < b.N; i++ {
			rv.validateEnumParsed(mixedEnum)
		}
	})
