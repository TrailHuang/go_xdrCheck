...
func testConditionValidation() {
	...
	testCases := []struct {
		...
		// 等于条件测试
		{"必须填写", []string{"", "5", "test"}, "if($1==5)", true, "条件满足，字段有值"},
		{"", []string{"", "5", "test"}, "if($1==5)", false, "条件满足，字段为空"},
		{"", []string{"", "8", "test"}, "if($1==5)", true, "条件不满足，字段为空"},

		// 不等于条件测试
		{"必须填写", []string{"", "8", "test"}, "if($1!=5)", true, "条件满足，字段有值"},
		{"", []string{"", "8", "test"}, "if($1!=5)", false, "条件满足，字段为空"},
		{"", []string{"", "5", "test"}, "if($1!=5)", true, "条件不满足，字段为空"},

		// 多个值条件测试
		{"必须填写", []string{"", "8", "test"}, "if($1==5,8)", true, "多值条件满足，字段有值"},
		{"", []string{"", "8", "test"}, "if($1==5,8)", false, "多值条件满足，字段为空"},

		// 字符串条件测试
		{"必须填写", []string{"", "test", ""}, "if($1==\"test\")", true, "字符串条件满足"},
		{"", []string{"", "test", ""}, "if($1==\"test\")", false, "字符串条件满足但字段为空"},
	}
